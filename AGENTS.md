...
- **kala-english-mode 只装 Claude Code / Codex**:辅助英语学习模式(对话内开关 + 两段式输出),两端均已实测;Cursor / OpenClaw 的等效触发未验证,安装器由 `CURSOR_SKIP` / `OPENCLAW_SKIP` 表跳过并清理历史副本。要扩展到新工具,先手工拷贝按四场景验证(开启确认 / 中文教表达 / 英文纠错 / 连续技术任务不掉格式),通过后再从对应跳过表移除。
  ...
  - **⛔ hook 只能注册到"已安装"路径,永远不能注册到仓库(工程文件夹)路径**:仓库是安装源,不是运行位置——agent 工具永远只应该跑 `~/.claude/skills/kala-english-mode/scripts/hook.mjs` 或 `~/.agents/skills/kala-english-mode/scripts/hook.mjs` 这两份"已安装"的副本,不能直接指向仓库里的 `skills/kala-english-mode/scripts/hook.mjs`。`wire-hooks.mjs` 内部固定按这两个已安装路径生成 command,不用 `import.meta.url` 取自己所在目录——不管你从仓库目录还是别的地方运行这个脚本本身,注册出来的路径都一样,且如果对应的已安装 `hook.mjs` 还不存在会直接跳过并提示先装。**这条是本机真实踩过的坑**:早期实现图省事直接注册了仓库路径,后来发现"正式安装"的 `SKILL.md`/`scripts/` 早就跟仓库脱节、停在几天前的旧版本,而 hook 却一直悄悄读着仓库里的新版本——两边不一致,容易误判"已经部署"。
  - **部署方式(顺序不能反)**:①先 `node install.mjs kala-english-mode` 把当前代码部署到两个已安装目录(这一步不能省,`wire-hooks.mjs` 依赖这些文件已经存在);②`node scripts/wire-hooks.mjs`,默认 dry-run 预览,加 `--yes` 才真正合并进 `~/.claude/settings.json` / `~/.codex/hooks.json`(读-改-写合并、按"是不是同一个 skill 的 hook"识别并原地替换旧路径,不会因为绝对路径不同就重复追加、幂等,自动备份原文件为 `*.kala-english-mode.bak`);③按系统对应的验证清单跑一遍(macOS 见 `SKILL.md` 里的验证描述,Windows 见 `references/windows-setup.md`)——跑完 `--yes` 不代表已验证,清单必须跑。修完 `scripts/` 里的代码后,要重新走一遍①②(重新 install 再重新 wire-hooks),不能只改仓库文件就当作已生效。
  - **⛔ Codex 端有一次性交互信任步骤,容易漏,且换路径要重新信任**:`hooks.json` 注册再正确,Codex 也要求在交互式会话里跑一次 `/hooks` 手动信任才会生效——`codex exec`(非交互)天生跳不过这一步,无法用脚本自动化验证。信任是按 hook 的 command 内容记的,**只要注册的绝对路径变了(比如从仓库路径迁移到已安装路径),就会被 Codex 当成新 hook,之前信任过也不算数,必须重新走一次 `/hooks`**——本机从仓库路径迁移到已安装路径后就实测复现过这个重置。Claude Code 端(user-level `settings.json`)没有这个限制,路径变了也是注册完立即生效——已用无头 `claude -p` 实测确认。
