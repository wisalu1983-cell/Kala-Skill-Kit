...
| **kala-english-mode** | 辅助英语学习模式:在 Claude Code / Codex 里用自然语言或 `/kala-english-mode` 开关,新 session 是否默认开启基础档由本机偏好决定。开启后每条回复两段式——先给英语表达反馈(中文输入→教地道说法,英文输入→纠错),再正常回答;基础档【回答】为英文 TL;DR + 中文详解,挑战档全英文。关闭只影响当前对话。 |
